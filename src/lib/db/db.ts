...
import { setScriptsInDb } from './all-script-set';

import type { Folder, Script } from '../types';
import { ALL_SCRIPTS } from '$lib/store/store-constants';
...
class FolderManager {
	...
	private async getScriptsFromFolders() {
		...
		const folders = (await this.db.select('SELECT * FROM folders')) as Folder[];

		if (!folders || folders.length === 0) {
			// if no folders in the db it allscripts sets it empty
			localStore.newStore(ALL_SCRIPTS, []);
	...
	async insertFolder(data: Folder): Promise<void> {
		if (this.db) {
			...
			await this.db.execute(`INSERT INTO folders (id, name, path) VALUES ($1, $2, $3)`, [
				folderId + data.name,
				data.name,
				data.path
			]);
	...
	async deleteFolder(id: string | number): Promise<void> {
		if (this.db) {
			...
			const data = (await localStore.getStore(ALL_SCRIPTS)) as Script[];
			const afterdelte = data.filter((folder) => folder.folderId !== id);
			console.log(afterdelte);
			await localStore.newStore(ALL_SCRIPTS, afterdelte);
		}
		await this.getScriptsFromFolders();
	}
